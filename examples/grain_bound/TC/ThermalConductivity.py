...
class ThermalConductivity(object):
    ...
    def read_size(self, relax_data, i, thickness, r=0, heatflux_direction=1):  # 热流方向 1 2 3
    ...
    def thermal_conductivity(self, result, TempGrad_fator=1):
        ...
        with open(self.result, "a+") as tc_k:
            ...
            print('Thermal conductivity:' + str(round(self.k, 5)), 'W/m-K\n')  # round(要输出的值,保留几位小数)
            tc_k.write(str(round(self.k, 5)))
            if self.case == 3:
                tc_k.write('\n')
            else:
                tc_k.write(' ')
