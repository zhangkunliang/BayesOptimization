...
def load_data():
    ...
    A = np.asarray(np.loadtxt('Cnb_test_data/datanew.csv', skiprows=0, delimiter=','))
    X = A[:, 0:9]
    t = -A[:, 9]
    return X, t
...
if __name__ == "__main__":
    ...
    g = r'Cnb_test_data/result-pre-min1.csv'
    id_str = []
    fx = []
    h_id_str = []
    h_fx = []
    ...
    for j in range(len(fx[0])):
        ...
        for k in range(npool):
            # m.append(id_str[k][j])
            m.append(str(-1.0 * fx[k][j]))
            # m.append(h_id_str[k][j])
            # m.append(str(-1.0*h_fx[k][j]))
        b = ','.join(m)
